#!/usr/bin/env python
"""Create a classification leaderboard for a BenchHub dataset (idempotent).

Clones the cifar10/beans pattern for any image-classification dataset that has
a `label` GT field with class names: contract = label_pred (label) +
label_topk_pred (label_list); metric = top_1_accuracy (+ top_5_accuracy when
the class count > 5). Auto-detects the input image field + label field + class
names from the dataset's DatasetField rows.

Usage:
    BENCHHUB_DATA_DIR=$HOME/.dtofbenchmarking \\
        ~/benchhub/.venv/bin/python scripts/create_classification_lb.py <dataset_id>

Prints a machine-readable line the submit step consumes:
    LB_RESULT lb_id=<id> repo=<hf_repo> input_field=<name> n_classes=<n>
or `LB_SKIP <reason>`.
"""
import os
import sys
...
sys.path.insert(0, '/home/ymatri/Git/BenchHub')   # beat any stray /tmp/app.py
os.environ.setdefault('BENCHHUB_DATA_DIR', os.path.expanduser('~/.dtofbenchmarking'))


def main():
    if len(sys.argv) < 2:
        print('LB_SKIP usage: create_classification_lb.py <dataset_id>'); return 2
    ...
    with A.app.app_context():
        ...
        # label GT field + its class names
        ...
        if img is None or lab is None:
            print(f'LB_SKIP dataset {ds_id} ({ds.name}) needs an image input + label GT field'); return 1
        try:
        ...
        # Reuse ANY benchmark LB already linked to this dataset — the earlier
        # sweep named some boards short (cifar100_benchmark) while ds.name is
        # uoft-cs__cifar100, so a name-only check would duplicate the board.
        ...
        if existing is not None:
            print(f'LB_RESULT lb_id={existing.id} repo={repo} input_field={img.name} '
        ...
        lb = Leaderboard(
            ...
            category=(ds.category or 'Vision/Image Classification'),
            required_pred_fields_json=json.dumps(contract),
            field_roles_json=json.dumps({img.name: "input", lab.name: "gt"}),
            summary_metrics='',
        )
        ...
        db.session.commit()
        print(f'LB_RESULT lb_id={lb.id} repo={repo} input_field={img.name} n_classes={n}  (created)')
        return 0
