...
def test_propose_classlabel_picks_top1_accuracy(dataset_classlabel_shape):
    ...
    assert 'def top1' in p['fallback_code']
...
def test_propose_depth_field_picks_three_metrics(db_session):
    ...
    assert names == ['a1', 'abs_rel', 'rmse']
    sort_dirs = {p['global_name']: p['sort_direction'] for p in proposals}
    assert sort_dirs['rmse'] == 'lower_is_better'
    assert sort_dirs['abs_rel'] == 'lower_is_better'
    assert sort_dirs['a1'] == 'higher_is_better'
    for p in proposals:
...
def test_propose_depth_visualization_emits_error_heatmap(db_session):
    ...
    assert len(viz) == 1
    v = viz[0]
    assert v['global_name'] == 'depth_error_heatmap'
...
def test_propose_skips_class_name_sidecar(dataset_classlabel_shape):
    ...
    proposals = _propose_metrics_for_dataset(dataset_classlabel_shape)
    assert {p['global_name'] for p in proposals} == {'top1'}
...
def test_propose_visualization_for_classlabel_is_confusion_matrix(
    ...
    assert len(viz) == 1
    v = viz[0]
    assert v['global_name'] == 'confusion_matrix'
    assert v['is_aggregated'] is True
    assert v['accepts_aggregated_inputs'] is True
    assert v['arg_mappings'] == {'gt': 'gt_label', 'pred': 'sub_label_pred'}
    assert 'def confusion_matrix' in v['fallback_code']
...
def test_metric_llm_generate_returns_none_without_api_key(monkeypatch):
    ...
    assert _llm_generate_metric_code('top1', 'classlabel') is None
...
def test_metric_llm_generate_rejects_response_with_wrong_function_name(monkeypatch):
    ...
    with patch('requests.post', return_value=_Resp()):
        out = _llm_generate_metric_code('top1', 'classlabel')
    assert out is None
...
def test_viz_llm_generate_returns_python_when_api_key_set(monkeypatch):
    ...
    assert 'def confusion_matrix(' in out
...
def test_auto_create_lb_attaches_metric_and_visualization_for_classlabel(
    ...
    assert [lm.global_metric.name for lm in lms] == ['top1']
    assert [lv.global_visualization.name for lv in lvs] == ['confusion_matrix']
...
def test_auto_create_lb_uses_existing_global_metric_when_named_match(
    ...
    assert GlobalMetric.query.filter_by(name='top1').count() == 1
...
def test_auto_create_lb_uses_existing_visualization_when_named_match(
    ...
    assert GlobalVisualization.query.filter_by(name='confusion_matrix').count() == 1
...
def test_auto_create_lb_falls_back_to_static_when_llm_unavailable(
    ...
    assert ok and lb_id
    gm = GlobalMetric.query.filter_by(name='mae').first()
    assert gm is not None
    assert 'def mae' in gm.python_code
...
def test_auto_create_lb_uses_llm_metric_code_when_api_key_set(
    ...
    assert ok and lb_id
    gm = GlobalMetric.query.filter_by(name='top1').first()
    gv = GlobalVisualization.query.filter_by(name='confusion_matrix').first()
...
def test_auto_finalize_persists_only_kept_proposals(
    ...
    resp = auth_client.post(
        ...
        data={
            ...
            'kept_metric_top1': '1',
            'metric_target_name_top1': 'top-1 accuracy (label)',
            'metric_sort_direction_top1': 'higher_is_better',
            # Note: kept_viz_confusion_matrix intentionally absent.
        },
        follow_redirects=False,
    )
    ...
    assert [lm.global_metric.name for lm in lb.leaderboard_metrics] == ['top1']
...
def test_auto_finalize_honors_user_edits_to_target_name_and_sort(
    ...
    resp = auth_client.post(
        ...
        data={
            ...
            'dataset_id': str(dataset_classlabel_shape.id),
            'kept_metric_top1': '1',
            'metric_target_name_top1': 'My renamed top1',
            'metric_sort_direction_top1': 'lower_is_better',
        },
        follow_redirects=False,
    )
    assert resp.status_code == 302
