...
- **Scope today: GT/input fields only.** Registered-kind *predictions* are a documented follow-up — `validate_submission_manifest` / `import_typed_submission` / `check_submission_matches_contract` still gate on `DTYPES` (and the standalone client can't know server-registered kinds at submit-validate time). A registered kind used by a public LB can't be deleted or made private (`_datatype_used_by_public_lb` guard).
